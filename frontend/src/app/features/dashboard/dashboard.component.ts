...
export class DashboardComponent implements OnInit {
  ...
  // --- NEW: Local signal to hold the real database records ---
  readonly recentUploads = signal<any[]>([]);

  ngOnInit(): void {
    // --- NEW: Fetch the first 6 documents directly from PostgreSQL ---
    this.documentService.getDocuments(0, 6).subscribe({
      next: (response) => {
        // We extract the 'content' array from the paginated Spring Boot response
        this.recentUploads.set(response.content);
      },
      error: (err) => {
        console.error('Failed to load recent documents from database', err);
      }
    });
  }
