...
// Import our new shared Dumb Component
...
@Component({
  ...
  imports: [
    RouterLink,
    DocumentCardComponent // DatePipe successfully removed from here!
  ],
    ...
    this.documentService.getDocuments(0, 6).subscribe({
      next: response => {
        const activeDocuments = response.content.filter(
          (document: DocumentUploadResponse) => document.status !== 'DELETED'
        );
